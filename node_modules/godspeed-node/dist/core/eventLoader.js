...
const rewiteRefsToAbsolutePath = (events) => {
    ...
    // deep copy
    logger_1.logger.info('Replacing $refs in events with definitions.');
    ...
    return Object.keys(deepCopyOfEvents).reduce((accumulator, eventKey) => {
        ...
        logger_1.logger.info('eventObject %o', eventObject);
        ...
        logger_1.logger.info('responses %o', responses);
        if (responses) {
            Object.keys(responses).forEach((responseCode) => {
                let responseContent = responses[responseCode].content;
                logger_1.logger.info('responseContent %o', responseContent);
                if (responseContent) {
